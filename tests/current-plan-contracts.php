...
$must=array(
 ['bootstrap','Version: 1.2.0-rc5'],['bootstrap',"define( 'RSV_CONTRACT_VERSION', 11 )"],['bootstrap',"'class-rsv-current-plan.php'"],['bootstrap',"'class-rsv-future30.php'"],['bootstrap',"'class-rsv-fresh20-hardening.php'"],['bootstrap',"'class-rsv-fresh-review-hardening.php'"],['bootstrap',"'class-rsv-third-review-hardening.php'"],
 ...
 ['admin','ORDER BY CASE reason_code'],['future',"'payment_or_donation_reason'=>false"],['future','medical_authority'],['future','apply_feed_runtime_preferences'],['future','authoritative_duration_seconds'],['future','MIN_CREATOR_RESEARCH_SAMPLE'],['future',"'language-version','File 11'"],
);
foreach($must as [$k,$n])if(false===strpos($f[$k],$n)){fwrite(STDERR,"Missing current-plan marker [$k]: $n\n");exit(1);}
echo "RC10/Future30 governing-plan contracts PASS under rc5\n";
