...
$latest = RSV_Helpers::cursor_encode( 'latest', '2026-08-06 12:00:00', '', 42, 'research' );
$decoded = RSV_Helpers::cursor_decode( $latest, 'latest', 'research' );
if ( is_wp_error( $decoded ) || 42 !== $decoded['id'] || 'latest' !== $decoded['sort'] ) exit( 1 );
if ( ! is_wp_error( RSV_Helpers::cursor_decode( $latest, 'recommended', 'research' ) ) ) exit( 1 );
$tampered = substr( $latest, 0, -1 ) . ( substr( $latest, -1 ) === 'a' ? 'b' : 'a' );
if ( ! is_wp_error( RSV_Helpers::cursor_decode( $tampered, 'latest', 'research' ) ) ) exit( 1 );
...
if ( ! is_wp_error( RSV_Helpers::cursor_decode( $latest, 'latest', 'materia-medica' ) ) ) exit( 1 );
if ( 'research' !== $decoded = RSV_Helpers::cursor_decode( $latest, 'latest', 'research' )['context'] ) exit( 1 );
