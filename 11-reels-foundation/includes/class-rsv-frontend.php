	...
	public function feed() {
		...
		<main class="rsv-shell" aria-labelledby="rsv-feed-title" data-rsv-sort="<?php echo esc_attr( $sort ); ?>" data-rsv-topic="<?php echo esc_attr( $topic ); ?>">
			...
			<form class="rsv-discovery-controls" method="get" action="<?php echo esc_url( home_url( '/reels/' ) ); ?>">
				<label><span><?php esc_html_e( 'Topic', RSV_TEXT_DOMAIN ); ?></span><select name="topic"><option value=""><?php esc_html_e( 'All approved topics', RSV_TEXT_DOMAIN ); ?></option><?php foreach ( RSV_Contracts::TOPICS as $allowed_topic ) : ?><option value="<?php echo esc_attr( $allowed_topic ); ?>" <?php selected( $topic, $allowed_topic ); ?>><?php echo esc_html( ucwords( str_replace( '-', ' ', $allowed_topic ) ) ); ?></option><?php endforeach; ?></select></label>
