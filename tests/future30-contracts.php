...
$must = array(
 ['bootstrap','Version: 1.2.0-rc5'], ['bootstrap',"define( 'RSV_CONTRACT_VERSION', 11 )"], ['bootstrap',"'class-rsv-future30.php'"], ['bootstrap',"'class-rsv-fresh20-hardening.php'"], ['bootstrap',"'class-rsv-fresh-review-hardening.php'"], ['bootstrap',"'class-rsv-third-review-hardening.php'"],
 ...
 ['future','rsv_translation_language_mismatch'], ['future','ReelQuizAttemptUpdated'], ['future','render_safe_tools'], ['future','ReelTranscriptProjectionReady'], ['future','ReelLanguageLinksReconciled'], ['future','authoritative_duration_seconds'], ['future','MIN_CREATOR_RESEARCH_SAMPLE'], ['future',"'language-version','File 11'"], ['future',"$validation_owner = 'F11-FUT-014' === $feature_id ? 'File 11' : $owner"],
);
...
$forbidden = array('raw media owner File 11','autonomous diagnosis','paid ranking advantage',"'language-version','translation-provider'");
foreach($forbidden as $needle) if(false!==stripos($f['future'],$needle)){fwrite(STDERR,"Forbidden Future30 claim: $needle\n");exit(1);}
echo "File 11 Future30 rc5 contracts PASS\n";
