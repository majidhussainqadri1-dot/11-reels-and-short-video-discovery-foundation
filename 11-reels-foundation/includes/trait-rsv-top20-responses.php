...
trait RSV_Top20_Responses_Trait {
	...
	public static function create_response( $source_public_id, $data ) {
		if ( ! RSV_Security::can( RSV_Contracts::CAP_SUBMIT, null, 'create_response' ) ) return RSV_Helpers::error( 'rsv_forbidden', __( 'You cannot create Reel responses.', RSV_TEXT_DOMAIN ), 403 );
		...
		if ( ! RSV_Security::can_view_reel( $source, 0 ) || ! $response || absint( $response['owner_id'] ) !== get_current_user_id() || absint( $source['id'] ) === absint( $response['id'] ) ) return RSV_Helpers::error( 'rsv_response_reel_invalid', __( 'Select a different eligible response Reel that you own.', RSV_TEXT_DOMAIN ), 422 );
		$context = self::context_row( absint( $source['id'] ) );
		if ( ! $context || empty( $context['allow_response'] ) ) return RSV_Helpers::error( 'rsv_response_disabled', __( 'The source publisher has not enabled responses for this Reel.', RSV_TEXT_DOMAIN ), 409 );
		...
		if ( $patient_case && ( ! $declared || ! apply_filters( 'rsv_patient_reuse_consent_valid', false, $source, $response, $data ) ) ) return RSV_Helpers::error( 'rsv_patient_reuse_consent_required', __( 'Verified patient-media reuse consent is required for this response.', RSV_TEXT_DOMAIN ), 422 );
		$attribution = RSV_Helpers::text( $data['attribution_text'] ?? sprintf( __( 'Response to “%s” by %s', RSV_TEXT_DOMAIN ), $source['title'], get_the_author_meta( 'display_name', absint( $source['owner_id'] ) ) ), 500 );
		global $wpdb;
		$public_id = RSV_Helpers::public_id( 'response' );
		$insert = $wpdb->insert( RSV_Helpers::table( 'responses' ), array( 'public_id' => $public_id, 'source_reel_id' => absint( $source['id'] ), 'response_reel_id' => absint( $response['id'] ), 'owner_id' => get_current_user_id(), 'attribution_text' => $attribution, 'patient_reuse_consent' => $declared ? 1 : 0, 'status' => 'review', 'version' => 1, 'published_at' => null, 'created_at' => RSV_Helpers::now(), 'updated_at' => RSV_Helpers::now() ), array( '%s','%d','%d','%d','%s','%d','%s','%d','%s','%s','%s' ) );
		if ( 1 !== $insert ) return RSV_Helpers::error( 'rsv_response_write_failed', __( 'The response could not be created. This Reel may already be linked as a response.', RSV_TEXT_DOMAIN ), 409 );
		...
		return array( 'id' => $public_id, 'status' => 'review' );
	}

	public static function publish_response( $public_id, $expected_version ) {
		$row = self::response_row( $public_id, true );
		if ( ! $row || ! RSV_Security::can( RSV_Contracts::CAP_PUBLISH, $row, 'publish_response' ) ) return RSV_Helpers::error( 'rsv_response_not_found', __( 'Response not found.', RSV_TEXT_DOMAIN ), 404 );
		if ( absint( $row['version'] ) !== absint( $expected_version ) ) return RSV_Helpers::error( 'rsv_version_conflict', __( 'This response changed. Refresh and try again.', RSV_TEXT_DOMAIN ), 409 );
		$source = RSV_Repository::find( absint( $row['source_reel_id'] ) );
		$response = RSV_Repository::find( absint( $row['response_reel_id'] ) );
		if ( ! RSV_Security::can_view_reel( $source, 0 ) || ! RSV_Security::can_view_reel( $response, 0 ) ) return RSV_Helpers::error( 'rsv_response_dependency_changed', __( 'The source or response Reel is unavailable.', RSV_TEXT_DOMAIN ), 409 );
		$context = self::context_row( absint( $source['id'] ) );
		if ( ! $context || empty( $context['allow_response'] ) ) return RSV_Helpers::error( 'rsv_response_disabled', __( 'Responses are no longer enabled for the source Reel.', RSV_TEXT_DOMAIN ), 409 );
		$source_video = RSV_File10::video( absint( $source['video_id'] ) );
		$patient_case = 'not_patient_case' !== ( $source_video['consent_status'] ?? '' );
		...
		global $wpdb;
		$ok = $wpdb->update( RSV_Helpers::table( 'responses' ), array( 'status' => 'published', 'published_at' => RSV_Helpers::now(), 'version' => absint( $row['version'] ) + 1, 'updated_at' => RSV_Helpers::now() ), array( 'id' => absint( $row['id'] ), 'version' => absint( $row['version'] ) ), array( '%s','%s','%d','%s' ), array( '%d','%d' ) );
		if ( 1 !== $ok ) return RSV_Helpers::error( 'rsv_response_publish_failed', __( 'The response could not be published.', RSV_TEXT_DOMAIN ), 500 );
		RSV_Helpers::audit( 'response', absint( $row['id'] ), 'publish', $row['status'], 'published' );
		RSV_Helpers::outbox( 'ReelResponsePublished', 'response', absint( $row['id'] ), array( 'public_id' => $row['public_id'] ) );
	...
	private static function response_dto( $row ) {
		$source = RSV_Repository::find( absint( $row['source_reel_id'] ) );
		$response = RSV_Repository::find( absint( $row['response_reel_id'] ) );
		if ( 'published' !== $row['status'] || ! RSV_Security::can_view_reel( $source, 0 ) || ! RSV_Security::can_view_reel( $response, 0 ) ) return null;
		$source_video = RSV_File10::video( absint( $source['video_id'] ) );
		$patient_case = 'not_patient_case' !== ( $source_video['consent_status'] ?? '' );
		...
		return array( 'id' => $row['public_id'], 'attribution' => $row['attribution_text'], 'published_at' => $row['published_at'], 'source' => RSV_Repository::public_dto( $source ), 'response' => RSV_Repository::public_dto( $response ) );
	}
	...
	public static function public_responses( $source_public_id, $limit = 20 ) {
		...
		global $wpdb;
		$rows = (array) $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . RSV_Helpers::table( 'responses' ) . " WHERE source_reel_id=%d AND status='published' ORDER BY published_at DESC,id DESC LIMIT %d", absint( $source['id'] ), min( 50, max( 1, absint( $limit ) ) ) ), ARRAY_A );
		$out = array(); foreach ( $rows as $row ) { $dto = self::response_dto( $row ); if ( $dto ) $out[] = $dto; }
		return $out;
	}
	...
	public static function value_insights( $owner_id ) {
		...
		$rows = (array) $wpdb->get_results( $wpdb->prepare(
			"SELECT r.id,r.public_id,r.title,
			 COUNT(DISTINCT i.viewer_hash) views,
			 COALESCE(SUM(i.completed),0) completions,
			 COALESCE(AVG(i.viewed_seconds),0) average_seconds,
			 COALESCE(SUM(i.swiped_rapidly),0) rapid_swipes,
			 COALESCE((SELECT SUM(v.source_opens) FROM $signals v WHERE v.reel_id=r.id),0) source_opens,
			 COALESCE((SELECT SUM(v.shares) FROM $signals v WHERE v.reel_id=r.id),0) shares,
			 COALESCE((SELECT SUM(v.natural_stops) FROM $signals v WHERE v.reel_id=r.id),0) natural_stops,
			 COALESCE((SELECT COUNT(*) FROM $reports rp WHERE rp.reel_id=r.id),0) harm_reports
			 FROM $reels r LEFT JOIN $impressions i ON i.reel_id=r.id
			 WHERE r.owner_id=%d GROUP BY r.id ORDER BY r.updated_at DESC LIMIT 200",
			absint( $owner_id )
		), ARRAY_A );
		$out = array();
		foreach ( $rows as $row ) {
	...
	public static function history_cursor( $user_id, $limit = 50, $cursor = '' ) {
		...
		global $wpdb;
		$p = RSV_Helpers::table( 'progress' ); $r = RSV_Helpers::table( 'reels' );
		$where = 'p.user_id=%d'; $args = array( $user_id );
		if ( $decoded ) { $where .= ' AND (p.updated_at<%s OR (p.updated_at=%s AND p.id<%d))'; $args[]=$decoded['primary']; $args[]=$decoded['primary']; $args[]=$decoded['id']; }
		$args[] = $limit + 1;
		...
		$has_more = count( $rows ) > $limit; if ( $has_more ) array_pop( $rows );
		$items = array();
		foreach ( $rows as $row ) {
			...
			if ( RSV_Security::can_view_reel( $row, $user_id ) ) { $dto=RSV_Repository::public_dto($row); $dto['available']=true; $dto['progress']=$progress; $items[]=$dto; }
			else $items[]=array('id'=>'unavailable-'.substr(hash('sha256',$row['public_id']),0,12),'title'=>__('Unavailable Reel',RSV_TEXT_DOMAIN),'available'=>false,'url'=>'','progress'=>$progress);
		}
