...
trait RSV_Top20_Context_Trait {
	public static function authorization_gate( $allowed, $capability, $object, $purpose, $claims ) {
		...
		if ( RSV_Contracts::CAP_PUBLISH === $capability && 'publish_reel' === $purpose && is_array( $object ) ) return ! is_wp_error( self::publication_gate( $object ) );
		return true;
	}

	public static function publication_gate( $reel ) {
		$context = self::context_row( absint( $reel['id'] ?? 0 ) );
		if ( ! $context || '' === trim( (string) $context['source_title'] ) || '' === trim( (string) $context['safety_summary'] ) ) return RSV_Helpers::error( 'rsv_source_safety_required', __( 'A reviewed source label and safety summary are required before publication.', RSV_TEXT_DOMAIN ), 422 );
		...
		if ( ! $has_caption && ! $exception ) return RSV_Helpers::error( 'rsv_caption_track_required', __( 'A caption track, transcript link or explicitly approved accessibility exception is required.', RSV_TEXT_DOMAIN ), 422 );
		return true;
	}
	...
	public static function preferences( $user_id = 0 ) {
		...
		$defaults = array(
			'user_id' => $user_id,
			'youth_safe' => self::mandatory_youth_mode( $user_id ),
			'history_paused' => false,
			'session_limit_minutes' => 15,
			'natural_stop_every' => 10,
			'late_night_reminder' => true,
			'version' => 0,
			'mandatory_youth_safe' => self::mandatory_youth_mode( $user_id ),
		);
		...
		return array(
			'user_id' => $user_id,
			'youth_safe' => self::mandatory_youth_mode( $user_id ) || ! empty( $row['youth_safe'] ),
	...
	public static function save_preferences( $data, $user_id = 0 ) {
		...
		$expected = isset( $data['version'] ) ? absint( $data['version'] ) : absint( $current['version'] );
		$values = array(
			'youth_safe' => self::mandatory_youth_mode( $user_id ) || ! empty( $data['youth_safe'] ) ? 1 : 0,
			'history_paused' => ! empty( $data['history_paused'] ) ? 1 : 0,
			'session_limit_minutes' => min( 60, max( 5, absint( $data['session_limit_minutes'] ?? 15 ) ) ),
			'natural_stop_every' => min( 25, max( 5, absint( $data['natural_stop_every'] ?? 10 ) ) ),
			'late_night_reminder' => ! empty( $data['late_night_reminder'] ) ? 1 : 0,
			'updated_at' => RSV_Helpers::now(),
		);
		global $wpdb;
		$table = RSV_Helpers::table( 'preferences' );
		if ( 0 === absint( $current['version'] ) ) {
			$values['user_id'] = $user_id; $values['version'] = 1;
			$ok = $wpdb->insert( $table, $values, array( '%d','%d','%d','%d','%d','%s','%d','%d' ) );
		} else {
			...
			$ok = $wpdb->update( $table, $values, array( 'user_id' => $user_id, 'version' => $expected ), array( '%d','%d','%d','%d','%d','%s','%d' ), array( '%d','%d' ) );
		}
		if ( 1 !== $ok ) return RSV_Helpers::error( 'rsv_preferences_write_failed', __( 'Preferences could not be saved.', RSV_TEXT_DOMAIN ), 500 );
		RSV_Helpers::audit( 'wellbeing_preferences', $user_id, 'update', '', '', 'User-controlled privacy and well-being settings', array(), $user_id );
	...
	public static function save_context( $reel, $data ) {
		...
		return RSV_DB::transaction( static function () use ( $wpdb, $table, $reel, $data, $source_title, $safety, $policy ) {
			...
			$ok = $existing ? $wpdb->update( $table, $values, array( 'reel_id'=>absint($reel['id']),'version'=>$expected ), array('%s','%s','%s','%s','%d','%s','%d','%s'), array('%d','%d') ) : $wpdb->insert( $table, array_merge( array('reel_id'=>absint($reel['id'])), $values ), array('%d','%s','%s','%s','%s','%d','%s','%d','%s') );
