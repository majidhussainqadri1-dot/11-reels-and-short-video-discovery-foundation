...
final class RSV_Helpers {
	...
	public static function label( $value ) {
		...
		$labels = array(
			...
			'harm' => __( 'Harm or urgent safety risk', RSV_TEXT_DOMAIN ), 'false-claim' => __( 'False or misleading claim', RSV_TEXT_DOMAIN ),
			'impersonation' => __( 'Impersonation', RSV_TEXT_DOMAIN ), 'privacy' => __( 'Privacy', RSV_TEXT_DOMAIN ),
			'abuse' => __( 'Abuse or harassment', RSV_TEXT_DOMAIN ), 'copyright' => __( 'Copyright', RSV_TEXT_DOMAIN ),
			'scam' => __( 'Scam or spam', RSV_TEXT_DOMAIN ), 'child-safety' => __( 'Child safety', RSV_TEXT_DOMAIN ), 'other' => __( 'Other', RSV_TEXT_DOMAIN ),
			'medical-claim' => __( 'Medical claim', RSV_TEXT_DOMAIN ), 'patient-privacy' => __( 'Patient privacy', RSV_TEXT_DOMAIN ),
			'harassment' => __( 'Harassment', RSV_TEXT_DOMAIN ), 'spam' => __( 'Spam', RSV_TEXT_DOMAIN ),
			'low' => __( 'Low risk', RSV_TEXT_DOMAIN ), 'medium' => __( 'Medium risk', RSV_TEXT_DOMAIN ), 'high' => __( 'High risk', RSV_TEXT_DOMAIN ), 'critical' => __( 'Critical risk', RSV_TEXT_DOMAIN ),
	...
	public static function cursor_encode( $sort, $primary, $secondary, $id, $context = '' ) {
		$payload = self::json_encode( array(
			'v' => 3,
	...
	public static function cursor_decode( $cursor, $expected_sort = 'recommended', $expected_context = '' ) {
		...
		if ( false === strpos( $cursor, '.' ) ) return self::error( 'rsv_cursor_invalid', __( 'The feed cursor is invalid or expired.', RSV_TEXT_DOMAIN ), 400 );
		list( $encoded, $mac ) = explode( '.', $cursor, 2 );
		$expected = hash_hmac( 'sha256', $encoded, wp_salt( 'nonce' ) );
		if ( ! hash_equals( $expected, (string) $mac ) ) return self::error( 'rsv_cursor_invalid', __( 'The feed cursor is invalid or expired.', RSV_TEXT_DOMAIN ), 400 );
		...
		if ( 3 !== (int) ( $data['v'] ?? 0 ) || ! $sort || $sort !== $expected_sort || $context !== sanitize_key( (string) $expected_context ) || empty( $data['i'] ) ) return self::error( 'rsv_cursor_invalid', __( 'The feed cursor is invalid or expired.', RSV_TEXT_DOMAIN ), 400 );
		if ( 'latest' === $sort && ! self::is_mysql_datetime( $data['p'] ?? '' ) ) return self::error( 'rsv_cursor_invalid', __( 'The feed cursor is invalid or expired.', RSV_TEXT_DOMAIN ), 400 );
		if ( 'recommended' === $sort && ( ! is_numeric( $data['p'] ?? null ) || ! self::is_mysql_datetime( $data['q'] ?? '' ) ) ) return self::error( 'rsv_cursor_invalid', __( 'The feed cursor is invalid or expired.', RSV_TEXT_DOMAIN ), 400 );
		return array( 'sort'=>$sort, 'context'=>$context, 'primary'=>(string)$data['p'], 'secondary'=>(string)($data['q']??''), 'id'=>absint($data['i']) );
	}
	...
	public static function audit( $object_type, $object_id, $action, $from = '', $to = '', $reason = '', $context = array(), $actor_id = null ) {
		...
		$result = $wpdb->insert(
			self::table( 'audit' ),
			array(
				'trace_id' => self::trace_id(), 'actor_id' => null === $actor_id ? get_current_user_id() : absint( $actor_id ),
				'object_type' => self::text( $object_type, 40 ), 'object_id' => (int) $object_id, 'action_name' => self::text( $action, 80 ),
				'from_state' => self::text( $from, 40 ), 'to_state' => self::text( $to, 40 ), 'reason' => self::textarea( $reason, 2000 ),
				'context_json' => self::json_encode( $context ), 'created_at' => self::now(),
			),
	...
	public static function outbox( $event_name, $object_type, $object_id, $payload = array() ) {
		...
		$result = $wpdb->insert(
			self::table( 'outbox' ),
			array(
				'event_id' => self::public_id( 'evt' ), 'event_name' => RSV_Contracts::event( $event_name ), 'object_type' => self::text( $object_type, 40 ),
				'object_id' => (int) $object_id, 'payload_json' => self::json_encode( $payload ), 'status' => 'pending', 'attempts' => 0,
				'available_at' => $now, 'lock_token' => '', 'locked_at' => null, 'last_error' => '', 'created_at' => $now, 'updated_at' => $now,
			),
	...
	public static function state_message( $code, $message, $status = 400 ) {
		return array( 'code'=>sanitize_key($code), 'message'=>(string)$message, 'status'=>(int)$status, 'trace_id'=>self::trace_id() );
	}
}
