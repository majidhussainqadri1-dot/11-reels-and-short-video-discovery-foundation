...
foreach ( array( 'stories', 'highlights', 'highlight_items', 'responses', 'preferences', 'value_signals', 'reel_context' ) as $table ) {
	if ( false === strpos( $top, "table( '$table' )" ) ) { fwrite( STDERR, "Missing table $table\n" ); exit( 1 ); }
}
foreach ( array(
	...
	if ( false === strpos( $top, $needle ) ) { fwrite( STDERR, "Missing Top-20 contract: $needle\n" ); exit( 1 ); }
}
